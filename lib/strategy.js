 ...
 */
 function Strategy(config, verify) {
   ...
   this._validRequestHosts = config.validRequestHosts
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  try {
    ...
    if(this._validRequestHosts) {
      if(req.headers['host']) {
        if(this._validRequestHosts.indexOf(req.headers['host']) === -1) {
          throw new Error('SymmetricTokenStrategy return invalid host')
        }
      } else {
        console.log('*** Seems you are using an old version of Express ***');
        console.log('Error: impossible to verify the host.');
      }
    }
